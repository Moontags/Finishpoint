...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-transparent px-3 pt-3 pb-2 sm:px-5 sm:pt-4 sm:pb-3 lg:px-8">
      <div className={`mx-auto max-w-7xl rounded-2xl px-4 py-3 transition-all duration-200 sm:px-6 sm:py-3.5 ${
        scrolled
          ...
          : "border border-transparent bg-white shadow-none"
      }`}>
        <div className="flex items-center justify-between gap-3 sm:gap-5">
          <Link href="/" className="flex min-w-0 shrink-0 items-center" aria-label="Etusivu">
            <Image
              ...
              width={160}
              height={80}
              className="h-[5rem] w-auto object-contain sm:h-[5.75rem]"
              priority
            />
          </Link>

          <nav className="hidden items-center gap-1 text-[14px] font-medium text-slate-500 md:flex">
                ...
                <div className="w-64 rounded-xl border border-slate-200 bg-white/90 p-2 shadow-lg backdrop-blur-xl">
                  {serviceNavigationOrder.map((slug) => (
                    <Link
                      key={slug}
                      href={`/${slug}`}
                      className="block rounded-lg px-3 py-2 text-[13px] text-slate-600 transition hover:bg-slate-100 hover:text-slate-900"
                    >
          ...
          <div className="mt-2 grid gap-2 rounded-xl border border-slate-200 bg-transparent p-2 backdrop-blur-xl">
            {serviceNavigationOrder.map((slug) => (
              <Link
                key={slug}
                href={`/${slug}`}
                className="rounded-lg px-3 py-2 text-[13px] font-medium text-slate-600 transition hover:bg-slate-100 hover:text-slate-900"
              >
                {services[slug].navLabel}
              </Link>
            ))}
